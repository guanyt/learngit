#include "i2c.h"
#include "qndriver.h"
UINT8 QND_ReadReg(UINT8 adr)
{	
    uint8_t byRdBuf;	
    I2C_Read(&byRdBuf,adr,1);
        return byRdBuf;
}
UINT8 QND_WriteReg(UINT8 adr, UINT8 value)
{
//        uint8_t byCmdBuf[2];
//    
//        byCmdBuf[0]      =adr;
//        byCmdBuf[1]      = value;
        I2C_Write(&value,adr,1);

        return 1;//xBspI2c_MMDirectWrite(&stWrI2cOp);
}
